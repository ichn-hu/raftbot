...
理想情况下，Phase 1 可以把 Slock runtime 理解成 bot package：
...
实际验证中，model list 可以由 daemon 提供；如果 Agent 创建页仍无法 select runtime，说明 UI/server 还有 installed/available runtime gating，而且 runtime id 可能必须来自 server 已知列表。

短期绕过方案：

- `ready.runtimes` 上报所有 server 已知 runtime，例如 `claude,codex,antigravity,kimi,copilot,cursor,gemini,opencode,pi`。
- `machine:runtime_models:detect` 对任意 runtime 都返回 bot implementation model，例如 `prod-db-operator / Production Database Operator`。
- 用户创建 Agent 时先选择任意已知 runtime，再选择 bot model。

长期方案：

- marketplace install 写入 bot runtime installed/available 状态。
- Server 原生支持 `raftbot-*` runtime id。
- Bot implementation 可以继续用 model 或 command manifest 区分。
